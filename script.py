...
class HashTagSearch(metaclass=ABCMeta):
    ...
    def extract_recent_tag(self, tag):
        ...
        #print(shared_data)
        media = shared_data['entry_data']['TagPage'][0]['graphql']['hashtag']['edge_hashtag_to_top_posts']['edges']
        #print(media)
        posts = []
        for i in range(len(media)):
            #json_object = json.load(media[i]['node'])
            for node in media[i]:
                post = self.extract_recent_instagram_post(media[i]['node'])
                posts.append(post)
            media = shared_data['entry_data']['TagPage'][0]['graphql']['hashtag']['edge_hashtag_to_media']['edges']
            for node in media[i]['node']:
                post = self.extract_recent_instagram_post(media[i]['node'])
                posts.append(post)
